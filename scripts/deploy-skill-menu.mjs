...
const source = await readFile(resolve(process.cwd(), "contracts/SkillMenu.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "SkillMenu.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["SkillMenu.sol"]?.SkillMenu;
...
console.log(`Deploying SkillMenu from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_SKILL_MENU_CONTRACT_ADDRESS to this contract address in Vercel.");
